...
- `@getphonenumber`查看用户是否授权成功，没成功拒绝访问
- 用户授权则调用`uin.login`的`code`去获取用户的`openid`和`sessionkey`
- 获取用户的个人信息，调用`uni.getUserInfo`（需要传递`openid、sessionkey、encryptedData、iv、avatarUrl、nickName`）
