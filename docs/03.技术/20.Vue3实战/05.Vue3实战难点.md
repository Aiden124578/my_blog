...
```js
<!-- 预览图片水印功能 -->
<div v-if="picVisible" class="pic-Modal" id="pic-Modal" @click="closePicModal" @mousewheel="handerPictu">
  <div id="pic-wrap"></div>
  <img :src="picUrl" alt="">
  <canvas id="picCanvas" />
</div>
...
const handerPictu = (e) => {
  ...
  let imgWidth = img.offsetWidth || img.width || img.clientWidth;
  let imgHeight = img.offsetHeight || img.height || img.clientHeight;
  if (e.deltaY > 0) {
    picWidth.value = `${imgWidth - 20}px`;
    picHeight.value = `${imgHeight - 20}px`;
  } else {
    picWidth.value = `${imgWidth + 20}px`;
    picHeight.value = `${imgHeight + 20}px`;
  }
